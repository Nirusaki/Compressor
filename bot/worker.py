...
async def dl_link(event):
    ...
    aa = kk.split(".")[-1]
    rr = "encode"
    bb = kk.replace(f".{aa}", " [FIERCENETWORK]")
    out = f"{rr}/{bb}"
    thum = "thumb.jpg"
    ...
    ds = await xxx.client.send_file(
        xxx.chat_id, file=ok, supports_streaming=true, thumb=thum, caption=bb
    )
    await nnn.delete()
...
async def encod(event):
    try:
        ...
        ds = await e.client.send_file(
            e.chat_id, file=ok, supports_streaming=True, thumb=thum, caption=bb
        )
        await nnn.delete()
        org = int(Path(dl).stat().st_size)
        com = int(Path(out).stat().st_size)
